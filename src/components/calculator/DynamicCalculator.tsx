...
import { SubjectRow, MobileSubjectRow } from './SubjectRow'
...
export function DynamicCalculator({ onCalculate }: Props) {
  ...
  try {
    ...
    const totalCredits = gradingPolicy.semester_based
      ? safeSemesters.reduce((sum, sem) => sum + (sem?.subjects || []).reduce((sSum, s) => sSum + (s?.credit || 0), 0), 0)
      : safeSubjects.reduce((sum, s) => sum + (s?.credit || 0), 0)
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-slate-500">
            ...
            <span className={`px-2 py-0.5 rounded-md border ${isDark ? 'bg-white/3 border-white/8' : 'bg-slate-50 border-slate-200'}`}>
              Total: {totalCredits.toFixed(1)} Cr
            </span>
        ...
        {gradingPolicy.semester_based ? (
          <div className="space-y-3">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <i className={`fa-solid ${!collapsed.has(sem.id) ? 'fa-chevron-up' : 'fa-chevron-down'} text-slate-500 text-xs`}></i>
                    </div>
                  </div>
                      ...
                      >
                        <div className="p-3 pt-0">
                          ...
                          <div className="mt-3 flex items-center justify-between">
                            ...
                            >
                              <i className="fa-solid fa-plus text-[10px]"></i> Add Course
                            </motion.button>
                          </div>
                        </div>
                      </motion.div>
            ...
            <motion.button
              onClick={addSemester}
              className={`w-full py-2.5 rounded-xl border-2 border-dashed flex items-center justify-center text-xs font-semibold transition-all ${
                isDark
                  ? 'border-white/10 text-slate-400 hover:border-violet-500/30 hover:text-violet-400 hover:bg-violet-500/5'
                  : 'border-slate-200 text-slate-500 hover:border-violet-400 hover:text-violet-600 hover:bg-violet-50'
              }`}
    ...
    console.error("DynamicCalculator render crash:", err)
    return (
      <div className="p-5 rounded-2xl border border-red-500/20 bg-red-500/5 text-red-400 text-xs space-y-2">
        <p className="font-bold flex items-center gap-1.5">
          <i className="fa-solid fa-circle-exclamation text-red-500"></i>
          Calculator Render Error
        </p>
        <p className="font-mono text-[10px] p-2.5 rounded-lg bg-black/10 border border-white/5">
